...
class ADODB_gtfw_ldap extends ADOConnection {
    var $databaseType = 'gtfw_ldap';
   var $dataProvider = 'gtfw_ldap';
   ...
   {
      if (preg_match("/^dn=\[(.*)\](\s*)filter=\[(.*)\]$/", $sql, $parts) > 0) {
         $dn = $parts[1];
         $filter = $parts[3];
      } else {
         return FALSE;
      }
      $rs = ldap_search( $this->_connectionID, $dn, $filter );
      return $rs;
   }
...
class ADORecordSet_gtfw_ldap extends ADORecordSet{

   var $databaseType = "gtfw_ldap";
   var $canSeek = false;
   var $_entryID; /* keeps track of the entry resource identifier */

   function ADORecordSet_gtfw_ldap($queryID,$mode=false)
   {
      if ($mode === false) {
