...
class Rest {
	...
	private function processRequest() {
		...
		if(isset($_GET['sub']) && isset($_GET['act']))
			$this->Controller=ucfirst($_GET['act']->mrVariable).ucfirst($_GET['sub']->mrVariable);
		else
			null;
		...
		switch($this->request['method']) {
			case 'get':
				...
				break;
			case 'post':
				$this->request['params'] = array_merge($_POST->AsArray(), $_GET->AsArray());
				break;
			case 'put':
	...
	public function process() {
		try	{			
			...
			if(!$method->isStatic()) {
				...
				$this->response = array("gtfwResult"=>$method->invoke($controller));
